...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPanel(ctx, deps) {
       ...
       * One host call.
       *
       * The Connection RPC answers with an envelope and rejects only on
       * transport faults, so both shapes are folded into the single
       * `{ok:…}` result the panel already speaks.
       */
      const hostCall = (method, args) => {
        let call;
        try {
          call = ctx.connection.rpc.call(CHANNEL, method, args || {});
        } catch (e) {
        ...
        return Promise.resolve(call).then(
          (r) => {
            ...
            const msg = r && r.error
              ? (r.error.message || r.error.code || 'host call failed')
              : 'host call failed';
    ...
    /** Required client services: the slot registry, the RPC carrier, the locale, the picker. */
    const inject = ['slots', 'connection', 'locale', 'uiWorkspace'];
        ...
        const timer = setTimeout(() => {
          ...
          callHost(ctx, 'bootstrap', {}).then((b) => {
            ...
            const step = () => {
              ...
              const sc = scopes[i];
              i += 1;
              callHost(ctx, 'check-updates', { projectPath: sc }).then((r) => {
                if (!alive) return;
    ...
    /**
     * One host call from outside React (the idle sweep), same envelope folding
     * as the panel's own bridge.
     */
    function callHost(ctx, method, args) {
      let call;
      try {
        call = ctx.connection.rpc.call(CHANNEL, method, args || {});
      } catch (e) {
      ...
      return Promise.resolve(call).then(
        (r) => {
          if (r && r.ok) return r.value;
          const msg = r && r.error ? (r.error.message || r.error.code || 'host call failed') : 'host call failed';
          return { ok: false, error: String(msg) };
        },
        (e) => ({ ok: false, error: String(e && e.message ? e.message : e) }),
      );
