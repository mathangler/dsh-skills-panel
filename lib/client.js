...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function createPanel(ctx, deps) {
      ...
      /**
       * Generation counter for search-description prefetching.
       *
       * A new search has to abandon the previous one's work: its late answers
       * would otherwise merge descriptions for a list that is no longer on
       * screen, and its requests would compete with the new list's.
       */
      let descGen = 0;

      /**
       * Details already fetched, keyed by result and destination.
       *
       * Collapsing a details panel and opening it again used to refetch, so the
       * spinner came back for content the panel had already shown. The key
       * includes the target and the project because `preview` also reports
       * where the skill would be installed, and that follows the destination.
       */
      const detailCache = {};
      let detailReq = '';
      ...
      function Panel() {
        ...
        const onSearch = () => {
          ...
          hostCall('search', { query: q }).then(function (r) {
            ...
            // Answers that arrive after another search started belong to a list
            // that is no longer on screen, so they are dropped, not merged.
            const gen = (descGen += 1);
            const merge = function (d) {
              if (gen !== descGen) return;
            ...
            const fetchDesc = function (x) {
              if (gen !== descGen) return Promise.resolve();
              return hostCall('describe', { items: [{ source: x.source, skillId: x.skillId }] })
                .then(merge, function () {});
            };

            // The first screenful goes out at once, so the rows the user is
            // looking at fill as fast as the site can answer...
            const first = items.slice(0, 6);
            for (const x of first) fetchDesc(x);

            // ...and the remainder drains through a few lanes. Every result is
            // its own request, so a long list would otherwise stampede the
            // site; three lanes fill a thirty-row list in a few seconds and
            // stay a polite client. Only the first six used to be requested at
            // all, which left every later row reading "loading" forever.
            const rest = items.slice(6);
            let cursor = 0;
            const pump = function () {
              if (gen !== descGen || cursor >= rest.length) return;
              const x = rest[cursor];
              cursor += 1;
              fetchDesc(x).then(pump);
            };
            const lanes = Math.min(3, rest.length);
            for (let w = 0; w < lanes; w += 1) pump();
        ...
        const onDetail = (item) => {
          ...
          setDetailKey(k); setSerr('');
          const ck = k + '\u0000' + target + '\u0000' + pp;
          if (Object.prototype.hasOwnProperty.call(detailCache, ck)) {
            setDetail(detailCache[ck]);
            return;
          }
          setDetail(null);
          detailReq = ck;
          hostCall('preview', { source: item.source, skillId: item.skillId, target: target, projectPath: pp })
            .then(function (r) {
              // A newer panel was opened while this was in flight.
              if (detailReq !== ck) return;
              if (r && r.ok) {
                const v = { ok: true, info: r };
                detailCache[ck] = v;
                setDetail(v);
              } else {
                // Not cached: a failure may be transient, so it is retried.
                setDetail({ ok: false, error: (r && r.error) || 'Could not read details.' });
              }
            })
            .catch(function (e) {
              if (detailReq !== ck) return;
              setDetail({ ok: false, error: String(e && e.message ? e.message : e) });
            });
        };
