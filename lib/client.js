...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Mark the settings nav row that belongs to this plugin.
     *
     * The `settings.section` registration contract carries only id, order and
     * label — there is no icon field — and the shell chooses the nav glyph from a
     * hardcoded map that cannot know a third-party section id, so the geometry
     * gear is its fallback. With no supported hook, the one self-contained option
     * is to mark the rendered row and let this bundle's own stylesheet swap the
     * glyph. Matching is by label text plus a leading icon, which survives the
     * dialog mounting late, sections being reordered, and a section count we do
     * not control — none of which a positional selector would.
     *
     * React owns this subtree, but only rewrites the attributes it renders, so a
     * marker we add stays put until the node is replaced; the caller re-marks on
     * mutation.
     *
     * @param doc - the owning document.
     * @returns the number of rows marked by this call.
     */
    function markSettingsNav(doc) {
      let marked = 0;
      const buttons = doc.querySelectorAll('button');
      for (const button of buttons) {
        if (button.hasAttribute(NAV_ATTR)) continue;
        const first = button.firstElementChild;
        if (first === null || first === undefined) continue;
        if (String(first.tagName).toLowerCase() !== 'svg') continue;
        ...
        if (label !== EN.title && label !== ZH.title) continue;
      ...
      function Panel() {
        ...
        const onSearch = () => {
          ...
          hostCall('search', { query: q }).then(function (r) {
            ...
            const top = items.slice(0, 6).map(function (x) { return { source: x.source, skillId: x.skillId }; });
            if (top.length === 0) return;
            hostCall('describe', { items: top }).then(function (d) {
              if (d && d.ok && d.descriptions) setDescs(d.descriptions);
            }).catch(function () {});
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        markSettingsNav(document);
        if (typeof MutationObserver !== 'function' || !document.body) return () => {};
        let pending = false;
        const observer = new MutationObserver(() => {
          if (pending) return;
          pending = true;
          // Coalesce bursts: a streaming conversation mutates the body
          // constantly, and this handler must stay off that path.
          setTimeout(() => {
            pending = false;
            // Cheap fast path: while our marker survives there is nothing to do.
            if (document.querySelector('button[' + NAV_ATTR + ']') !== null) return;
            markSettingsNav(document);
          }, 300);
        });
