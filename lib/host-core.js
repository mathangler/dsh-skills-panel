...
export function createHandlers(ctx, options = {}) {
  ...
  async function ensureTree(source) {
    ...
    for (const ref of REFS) {
      ...
      // Read the stamp back rather than trusting `Date.now()`: the generation
      // is the meta file's mtime, because that is the clock the TTL reads.
      const stamp = await stat(metaPath);
      return { root, branch: ref, at: stamp.mtimeMs };
    }
