...
/**
 * How long a description resolved from skills.sh may be reused.
 *
 * Descriptions are per search result and change rarely, so they are worth
 * caching, but not forever: a skill that edits its frontmatter should show the
 * new text within a session rather than only after a restart.
 */
const DESC_TTL_MS = 10 * 60 * 1000;

/**
 * Per-item deadlines for the two description sources.
 *
 * A description is decoration on a search result, so it must never be able to
 * hold the list up. Each lookup carries its own deadline and the caller fans
 * them out in parallel, so one unreachable repository costs one blank line
 * instead of the whole panel.
 */
const DESC_TIMEOUT_MS = 8000;
const DOC_TIMEOUT_MS = 15000;

/** Hard ceiling on a skills.sh page read, so a pathological response is bounded. */
const DESC_MAX_BYTES = 1024 * 1024;
...
/** Decode the handful of HTML entities that can appear inside a meta tag. */
function decodeEntities(s) {
  return String(s)
    ...
    // Last, so an escaped entity survives one pass rather than two.
    .replace(/&amp;/g, '&');
}

/**
 * The description a skills.sh skill page advertises for itself.
 *
 * The page renders the skill's own frontmatter description into the standard
 * meta tags, which makes this the cheapest possible source: one request, no
 * GitHub quota, no archive. It is truncated by the site at roughly 160
 * characters, which is the right length for a search result and is exactly why
 * the details view still fetches the real file.
 */
function metaDescription(html) {
  ...
  const pick = (re) => {
    for (const tag of tags) {
      ...
      if (!m) continue;
      const v = decodeEntities(m[1] !== undefined ? m[1] : m[2]).trim();
      if (v !== '') return v;
    }
    return '';
  };
  return (
    pick(/\bname\s*=\s*["']?description\b/i) ||
    pick(/\bproperty\s*=\s*["']?og:description\b/i) ||
    pick(/\bname\s*=\s*["']?twitter:description\b/i)
  );
}

/**
 * Read a response until the description is in hand, then hang up.
 *
 * The tag is not where one would expect it. On a skills.sh skill page it sits
 * at roughly byte 119,000 of 124,000 — the flight payload is emitted ahead of
 * it — so stopping at `</head>` or at a small byte budget truncates before the
 * tag is ever reached, which is precisely how an earlier revision of this
 * function silently returned nothing for every live page while passing its
 * tests. Stopping on the extracted value instead is correct for this layout and
 * still cheap for a page that happens to put the tag early.
 */
async function readUntilDescription(res, max) {
  if (res.body === undefined || res.body === null || typeof res.body.getReader !== 'function') {
  ...
  try {
    for (;;) {
      ...
      // The cheap guard keeps the full parse off the per-chunk path.
      if ((out.indexOf('description') !== -1 && metaDescription(out) !== '') || out.length >= max) break;
    }
  } catch (e) {
    /* keep whatever arrived before the stream failed */
  }
  try {
    await reader.cancel();
  } catch (e) {
...
export function createHandlers(ctx, options = {}) {
  ...
  const descCache = {};
  const docCache = {};
  /** `source` -> `{ at, paths }`, the GitHub tree listing used to locate SKILL.md. */
  const treeCache = {};
  ...
  /**
   * One description, from the skills.sh skill page.
   *
   * This is what the search list uses: no GitHub quota, no archive, no
   * extraction. The page is a static render of the skill's own frontmatter.
   */
  async function siteDescription(id) {
    const ctl = new AbortController();
    const timer = setTimeout(() => ctl.abort(), DESC_TIMEOUT_MS);
    try {
      const res = await fetcher('https://www.skills.sh/' + id, {
        redirect: 'follow',
        signal: ctl.signal,
        headers: { accept: 'text/html' },
      });
      if (!res || res.ok !== true) return { ok: false };
      const text = metaDescription(await readUntilDescription(res, DESC_MAX_BYTES));
      return text === '' ? { ok: false } : { ok: true, description: text };
    } catch (e) {
      return { ok: false };
    } finally {
      clearTimeout(timer);
  ...
  /** One GitHub API call, with its own deadline. */
  async function githubJson(url, ms) {
    const ctl = new AbortController();
    ...
    try {
      const res = await fetcher(url, {
        redirect: 'follow',
        signal: ctl.signal,
        headers: { accept: 'application/vnd.github+json', 'user-agent': 'dsh-skills-panel' },
      });
      if (!res || res.ok !== true) {
        return { error: 'GitHub API returned HTTP ' + (res ? res.status : '?') };
      }
      return { value: JSON.parse(String(await res.text())) };
    } catch (e) {
  ...
  /**
   * Every `SKILL.md` path in a repository — one API call per repository.
   *
   * The archive is the wrong instrument for reading one small text file: a
   * skill repository can be megabytes, and all that is wanted here is a path.
   * `api.github.com` answers with the file listing instead, and the listing is
   * then reusable for every skill that lives in that repository.
   */
  async function githubSkillPaths(source) {
    const hit = treeCache[source];
    ...
    if (r.error !== undefined || r.value === undefined) return { error: r.error || 'no tree' };
    const tree = Array.isArray(r.value.tree) ? r.value.tree : [];
    const paths = tree
      .filter((e) => e && e.type === 'blob' && typeof e.path === 'string')
      .map((e) => e.path)
      .filter((p) => p === 'SKILL.md' || p.endsWith('/SKILL.md'));
    const rec = { at: Date.now(), paths };
    treeCache[source] = rec;
    return rec;
  }

  /**
   * Where `skillId`'s SKILL.md sits among a repository's paths.
   *
   * The containing directory is normally named after the skill. A repository
   * holding exactly one skill is unambiguous whatever its folder is called, so
   * those two rules cover the real layouts without fuzzy matching.
   */
  function pickSkillPath(paths, skillId) {
    const want = String(skillId).toLowerCase();
    for (const p of paths) {
      const parts = p.split('/');
      if (parts.length >= 2 && parts[parts.length - 2].toLowerCase() === want) return p;
    }
    if (paths.length === 1) return paths[0];
    for (const p of paths) if (p === 'SKILL.md') return p;
    return '';
  }

  /** A skill's SKILL.md through the GitHub API, never touching the archive. */
  async function githubDoc(source, skillId) {
    const t = await githubSkillPaths(source);
    if (t.error !== undefined) return { error: t.error };
    const p = pickSkillPath(t.paths || [], skillId);
    if (p === '') return { error: 'no SKILL.md for "' + skillId + '" in ' + source };
    const url =
      'https://api.github.com/repos/' +
      source +
      '/contents/' +
      p.split('/').map(encodeURIComponent).join('/');
    const r = await githubJson(url, DOC_TIMEOUT_MS);
    if (r.error !== undefined || r.value === undefined) return { error: r.error || 'no content' };
    const v = r.value;
    if (typeof v.content !== 'string' || v.encoding !== 'base64') {
      return { error: 'unexpected contents payload for ' + p };
    }
    return {
      ok: true,
      path: p,
      content: Buffer.from(v.content.replace(/\s+/g, ''), 'base64').toString('utf8'),
    };
  }

  /**
   * A full SKILL.md for the details view, preferring the single-file API.
   *
   * The archive stays as the fallback on purpose: it is the only source that
   * also yields the resolved branch and the on-disk directory, and it is the
   * path an install needs regardless, so a repository the API cannot address
   * still works.
   */
  async function fetchDocFast(source, skillId) {
    const key = source + '/' + skillId;
    if (docCache[key] !== undefined) return docCache[key];

    const g = await githubDoc(source, skillId);
    if (g.ok === true) {
      const suffix = '/SKILL.md';
      const out = {
        ok: true,
        name: parseSkillName(g.content, skillId),
        rel: g.path.endsWith(suffix) ? g.path.slice(0, -suffix.length) : '',
        branch: 'HEAD',
        content: g.content,
        dir: '',
      };
    ...
    return fetchDoc(source, skillId);
  }
  ...
  return {
     ...
     *
     * Callers may send one item to fill a single card or many at once; either
     * way the lookups run in parallel and each carries its own deadline, so a
     * slow repository delays only its own line. The archive is deliberately not
     * involved: resolving a description used to mean downloading and extracting
     * a whole repository, which for a large one never finished at all.
     *
     * Only successes are cached. A description that failed because the network
     * blinked should be retried on the next search, not remembered as absent
     * for the life of the process.
     */
    async describe(args) {
      const items = Array.isArray(args && args.items) ? args.items.slice(0, 24) : [];
      const out = {};
      await Promise.all(
        items.map(async (it) => {
          const source = String(it && it.source !== undefined ? it.source : '');
          const skillId = String(it && it.skillId !== undefined ? it.skillId : '');
          if (source === '' || skillId === '') return;
          const key = source + '/' + skillId;

          const hit = descCache[key];
          if (hit !== undefined && Date.now() - hit.at < DESC_TTL_MS) {
            out[key] = hit.rec;
            return;
          }
          let rec = { ok: false };
          try {
            rec = await siteDescription(source + '/' + skillId);
          } catch (e) {
            rec = { ok: false };
          }
          if (rec.ok === true) descCache[key] = { at: Date.now(), rec };
          out[key] = rec;
        }),
      );
    ...
    async update(args) {
      ...
      try {
        ...
        // The listing is cached per repository, so a skill that moved folder
        // would otherwise keep resolving to its old location.
        delete treeCache[String(e.source)];
    ...
    async preview(args) {
      const a = args || {};
      const tr = await targetRoot(a);
      // Single-file API first, archive only as the fallback: the user is
      // waiting on this one click, and the archive can be megabytes.
      const d = await fetchDocFast(String(a.source), String(a.skillId));
      ...
      return {
        ...
        files: [],
        skipped: [],
        destination: tr.error ? null : join(tr.root, nm),
